...
#define IMAGEH  150 //行 HEIGHT 待采集摄像头图像高度行数
#define IMAGEW  250  //列 WIDTH  待采集摄像头图像宽度列数  //注意 一次DMA传输最大512个字节 RGB565一个像素点2个字节，所以图像宽度最好不要超过256 

#define LCDH    60  //OLED显示的行数
#define LCDW    80  //OLED显示的列数

#define SCL_Out     DDRB9=1      //配置输出作为SCL_Out
#define SDA_Out     DDRB8=1      //配置作为输出作为SDA_Out
#define SDA_In      DDRB8=0      //配置作为输入作为SDA_In

#define SCL_High    PTB9_OUT=1   //配置输出高电平
#define SCL_Low     PTB9_OUT=0   //配置输出低电平
#define SDA_High    PTB8_OUT=1   //配置输出高电平
#define SDA_Low     PTB8_OUT=0   //配置输出低电平
#define SDA_Data    PTB8_IN      //读取引脚上的引脚状态
...
extern u8 Image_Data[IMAGEH][IMAGEW * 2];  //图像原始数据存放
