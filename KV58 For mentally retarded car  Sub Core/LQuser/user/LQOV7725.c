...
u8 Image_Data[IMAGEH][IMAGEW * 2];  //图像原始数据存放
...
{     
  //行中断PTD13 
  if((PORTB_ISFR & 0x800)) //行中断 PTD13 (1<<13) 
  { 
    PORTB_ISFR |= 0x800;   //清除中断标识 
    // 用户程序            
    DMATransDataStart(DMA_CH4,(uint32_t)(&Image_Data[Line_Cont][1]));   //开启DMA传输 
  ...
  //场中断PTD14
  if((PORTB_ISFR & 0x10000))//场中断 PTD14 (1<<14)
  {
    PORTD_ISFR |= 0x10000;  //清除中断标识  
    // 用户程序 
    Line_Cont = 0;         //行计数清零
...
void TFTSPI_Show_Pic3(uint8_t xs,uint8_t ys,uint8_t w,uint8_t h,uint8_t ppic[IMAGEH][IMAGEW * 2]) 
{
    unsigned int i,j;	
    TFTSPI_Set_Pos(xs,ys,xs+w-1,ys+h);
    for(i=h*2-2;i>0;i-=2)  //隔一个取一个
    { 	
      for(j=0; j<w*2; j+=2) //隔一个取一个
      {
        TFTSPI_Write_Word((ppic[i][2*j]<<8)+(ppic[i][2*j+1])); //高位在前，且两个数据组合成一个16位数据表示像素值  
...
{     
  ...
  //摄像头寄存器设置
    ...
    {
        TFTSPI_P8X16Str(0,0,(u8*)"7725 dpi!!", u16RED, u16BLUE);                      //摄像头识分辨率设置过大，停止运行
        while(1);
    }
    ...
    {     
        sprintf(text,"%03d",ack);
        TFTSPI_P8X16Str(0,0,"NG", u16RED, u16BLUE);                      //摄像头识别失败，停止运行
        while(1); 
    } 
    else                                                   //芯片ID正确
    {
        TFTSPI_P8X16Str(0,0,(u8*)"OV7725 OK", u16RED, u16BLUE);
    }
...
//    vstart = 0x03U << 1U;   //图像垂直开始位置
  ...
  //GPIO口初始化
  EXTI_Init(GPIOB,11,rising_down);   //行中断
  EXTI_Init(GPIOB,16,falling_up);    //场中断  
...
//    DMA_PORTx2BUFF_Init (DMA_CH4, (void *)&PTD_BYTE0_IN,(void*)Image_Data, PTD12, DMA_BYTE1, (IMAGEW * 2 > 511 ? 511:IMAGEW * 2), DMA_rising); //一次DMA传输最大512个字节 
    DMA_PORTx2BUFF_Init (DMA_CH4, (void *)&PTB_BYTE0_IN,(void*)Image_Data, PTB10, DMA_BYTE1, (IMAGEW * 2 > 511 ? 511:IMAGEW * 2), DMA_rising); //一次DMA传输最大512个字节 
...
__ramfunc void Get_Use_Image(void)
{
  ...
  {
    for(j = 0;j < IMAGEW; j+=4)  //320列  每4列采一列
    {        
...
void SCCB_Init(void)
{
  GPIO_Init(GPIOB, 9,GPO,1);//配置为GPIO功能
  PORTB_BASE_PTR->PCR[9] |= 0x03; //上拉
  GPIO_Init(GPIOB, 8,GPO,1);//配置为GPIO功能 
  PORTB_BASE_PTR->PCR[8] |= 0x03; //上拉
}
